...
`https://jpfoodmap.com/`。壳不直接读写页面的 localStorage，收藏仍由网页管理；只有用户选择的 JSON 备份会经过原生文件选择器。不管
...
版本与网站同步：**3.1.1**（`versionCode 30101`）。网站发版，APP 跟着发。
...
- 其次：**上一版是用另一把签名钥匙签的**。安卓不允许换证书升级。先「设置 → 应用 →
  Japan Foodmap」检查已安装版本，优先找回原签名安装包覆盖升级。**不要直接卸载。**
  卸载或清除应用数据会一起删除此 WebView 的本地收藏、隐藏项、自建点、子收藏夹、
  登录会话与设置。只有确认另一设备已有最新完整数据，或已导出并验证可读 JSON 备份后，
  才考虑卸载。重新登录只能恢复已经成功同步到云端的部分。
- 如果你同时装过 **debug 版**（`com.fredhli.jpfoodmap.debug`，图标一样但是是另一个 APP），
  它和正式版各有独立的本地存储，也不会互相覆盖；删除测试版前同样先确认其中没有独有数据。
...
- 收藏仍由网页的 localStorage 与云端同步管理；壳不直接读取这些存储。WebView 的本地数据和会话属于此 APP，卸载会删除。
- **JSON 备份**：菜单 → 导出，选择系统文件目的地；只有实际写入完成才提示成功。导入由系统文件选择器读取用户选定的 JSON（最多 2 MiB）。取消不会改收藏或显示成功。备份格式与网页相同，不增加存储权限。
- 手机浏览器与此 APP 的本地存储相互独立，去浏览器导出不能代替备份 APP 内尚未同步的改动。
