...
(`ui-captures-2026-09-10/redesign/demo-4.0`) while the business layer ships
unchanged. `src/tabelog/ui/` is the new front-end source; `map.py` inlines it
into `docs/index.html` at build time (single-file deployment and the service
worker's atomic-install contract are unchanged).

### Changed

- **Presentation replaced, business kept.** `ui/js/business.js` is generated
  verbatim from `FILTER_JS_TEMPLATE` (sync engine, storage, sub-collections,
  import/export, auth, share, filter matcher, sort, l10n, emoji, SW/install
  plumbing) with a small, listed set of seams; `ui/js/adapter.js` is the one
  seam between it and the new modules. Every `localStorage` key, the KV blob,
  the Worker API and every `window.__*` test hook are unchanged.
  `audit_outputs/4.0.0-impl/PORT-PLAN.md` carries the classification.
- **Layout breakpoints** are DESIGN.md LAY-01's: narrow < 750 <= mid < 1100
  <= wide, decided once in `core.js` and applied through `html[data-mode]`.
  `scripts/verify_build.py`'s `breakpoints` check follows.
- **UI copy** is keyed Simplified strings in `ui/i18n/ui-strings.json`
  (en + ja hand-written, tw generated by `to_trad()`); business strings keep
  the run-based `TEXT_*_MAP`. The `i18n` gate excludes the UI-table runs from
  the missing-translation punch list because that table translates them.
- Rollback: `TABELOG_LEGACY_UI=1 uv run python src/tabelog/scrape/map.py`
  builds the 3.2.x page from the constants map.py still carries.

### Added

- **`ui-bundle`, a fourteenth build-gate check.** The front end is now ten
  source files inlined into one page in a fixed order, so the new ways to
  ship a broken page are structural. `scripts/verify_build.py` asserts that
  every `shell.html` slot reached the page, that the load order really is
  business → core → adapter → the six modules → `Adapter.start()`, that all
  six modules register between the adapter and the boot call, that no
  `__PLACEHOLDER__` survived, and that the UI i18n tables arrived with
  matching `en`/`ja` key sets.
- **Emission guards in `map.py`.** `read_ui()` refuses a `.js` file
  containing a literal `</script>` or a `.css` file containing `</style>`
  (inlined, either one ends the tag and the rest of the bundle becomes
  text); the build refuses to write `docs/index.html` while any placeholder
  token is still unsubstituted; `node --check` runs over every filled script
  and per-file byte counts are printed, both as warnings so a half-written
  module does not block a build.
- **`data/i18n` key-set parity** and half-translated `ui-strings.json` keys
  are reported at build time. A UI key with an `en` but no `ja` no longer
  gets the punch-list exemption — its runs fall back to the ordinary check
  instead of hiding behind a table that cannot translate them.

### Owner review (post-integration)

Four changes made after the owner reviewed the built page, each verified
against the four target viewports before release.

- **「全清」on 晚餐价格 / 菜系 did the opposite of its label.** The adapter
  substituted the full key list whenever the set was empty, so the empty set
  meant both "default" and "cleared" and the button silently re-selected
  everything: clear three buckets, press 全清, and all 7,860 rows came back.
  The sets are literal now — the default is materialised to every key and an
  empty one reaches `passesFilter` as an empty `pSet` / `gAny:false`, which is
  how 3.2.x produced no results (`// Genre filter: OR across selected
  categories. If none checked, hide all`). The checkboxes are honest again:
  "nothing ticked" no longer shows everything. A legacy `tabelog.filterState`
  with everything unchecked is restored as unchecked rather than as "all".
- **A detail card closed with × threw away your place in the result list.**
  `nav.back()` (Back / Esc) emitted `nav:restore` and was fine; the explicit ×
  emitted only `detail:close`, which nobody consumed, so all four widths reset
  to the top of the list. NAV-01 keeps the source id "for position restore
  only" on this path too. The list now restores from a close anchor that the
  scroll handler cannot overwrite, pumped until the column or sheet is laid
  out again. Back / Esc keep the trigger-row focus correction they had.
- **The ♡ on a result row opens the collection picker** instead of saving
  straight away, matching the detail card's main action: every user has a
  默认收藏夹 and their own collections sit beside it. The `F` shortcut and the
  saved-row ⋯ menu's 取消收藏 still act immediately — those are the fast paths.
- **百名店 reads Hyakumeiten in English again**, not "Top 100".
  `ui/i18n/ui-strings.json` had drifted from `data/i18n/en.json`.

### Fixed (integration pass)

Found while re-pointing the test suites at the 4.0 page — each one is a real
defect the suites caught, not a test adjustment.

- **The Back button could stop working on a phone.** `nav.activeLayers()`
  dropped the `browse` layer while a detail was open, so opening a card from
  the result panel left the history stack the same length, nothing was pushed,
  and the Back that closed the card consumed the panel's only entry: the next
  Back did nothing at all, and the one after it left the site. `nav.sync()`
  now counts the history entries it has actually pushed rather than comparing
  stack lengths, which also fixes the wide→narrow rotation case, and
  `nav.stamp()` keeps `history.state` describing the stack as it is now
  (`history.go()` is asynchronous, so a quick close-then-open used to leave an
  entry naming a layer that had already been dismissed).
- **A closed detail card could leave the keyboard with nowhere to be.** A card
  opened from search, from a marker or from a deep link has no source row, so
  nothing put the caret back and it sat on `<body>`. `containers` now places it
  on the panel header — after giving the list module its chance to return it to
  the source row, and standing down the moment the reader presses anything.
- **Focus was dropped by ordinary repaints.** A list-window repaint
  (`innerHTML`), a module root being re-parented between hosts, and a sheet
  header rewrite each sent the caret to `<body>`; the first two silently undid
  NAV-02's restore. All three now carry focus across.
- **NAV-02's restore ran too early.** It waited for the list to have a width
  but not for the panel's height transition to finish, so on a short screen it
  restored against a viewport that was about to change and could land hundreds
  of pixels out; it also gave up when the anchor row was outside the painted
  band instead of re-targeting. It now waits for a stable height, converges
  against the row's real rectangle, and guarantees the row you came from is on
  screen.
- **A tap on empty map no longer dismisses nothing.** The 3.2.x affordance was
  missing; it is back, as a dismissal (`closeDetail`), leaving no history entry.
- **A rail-layer fetch failure no longer switches the layer off.** The toggle
  stays where the reader put it and the failure is reported through
  `layers.error` with a 重试 that refetches (`layers:retry` /
  `MapMod.retryRail()`), instead of the control appearing to flip itself.
- **Sub-collections could be created but never renamed or deleted.** The Saved
  tab's collection groups now carry their own ⋯ (重命名收藏夹 / 复制清单文本 /
  删除收藏夹) — `act.renameList` and `act.deleteList` existed with no way in.
- **Signing out could not clear this device.** The account panel now asks in
  two steps (退出，保留本设备数据 / 退出并清除本设备数据 / 取消); before this
  `act.signOut(false)` was the only reachable path, and Business deliberately
  skips its own `confirm()` when given an options object.
- **A form's 保存 could sit under the software keyboard.** `#modal-root` was
  laid out on the layout viewport, so a bottom-sheet modal's `bottom: 0` was
  behind the keyboard. It is laid out on the usable rect `U` now, which already
  tracks `visualViewport`.
- **Junk could reach the synced blob.** `act.toggleFav` / `act.setBlack` /
  `act.batch` accepted any value and wrote it into `omakase_state_cache_v2`;
  they now refuse anything that is not a known restaurant id.
- **A hidden landmark came back on the next load.** `Business.api.hiddenBuiltinIds`
  was only ever maintained, never built — 3.2.x seeded it inside
  `renderFavoritesBuiltin`, which 4.0 dropped. The store seeds it where it is
  constructed.
- `距离最近` now shows the distance on each row, `编辑书签` / `删除书签` /
  `隐藏景点` are reachable from a tap on the pin or landmark itself, the
  documented `J` / `K` / `F` / `X` shortcuts are implemented rather than only
  advertised, `act.editPin` edits a pin in place (the id is never rewritten,
  because every `category:'meta'` member row references it), `showToast` keeps
  an `action`, `defaultState()` declares the slices the adapter seeds, and the
  PWA splash is white instead of the 3.2.x cream.

### Fixed

- **`build_emoji_cache.py` now scans `src/tabelog/ui/`.** Its text targets
  were `map.py`, the popups payloads and `data/i18n`; with the front end
  moved out, every emoji a module introduced was missing from the manifest
  and cost each visitor an `emojicdn` round-trip on first render. Five
  glyphs the 4.0 UI uses (♦️ ⚙ ✔️ 🌐 🍴) are pre-cached as a result.
- **The emoji manifest no longer loses entries.** It was rebuilt from the
  freshly collected set each run, so a character dropped out the moment the
  last text mentioning it changed — while its PNG stayed on disk (M-060
  never deletes one). Entries whose file is still present are carried
  forward. 🔗 had already been lost this way.
- **A CJK character class in module JS is no longer a translation demand.**
  `/[\u3040-\u30ff\u3400-\u9fff…]/` in `overlays.js` spells its own range
  boundaries, and the build-time CJK scan read those boundary characters as
  three untranslated UI strings. Bracketed groups containing a CJK-to-CJK
  range are stripped before the scan, the same way the runtime tokenizer's
  own literal already was.

### Verified

- Final build gate: all 14 named checks / 20 `ok` lines hold — popups,
  popup-slots, restaurants, row-fields, storage-keys, subcollections,
  breakpoints, ui-bundle, page zoom, sw, manifest, about (v4.0.0), i18n
  (EN 422 / JA 422, under the 488/489 baseline ceiling), csv. Full log and
  the local-preview / test-suite record are in
  `audit_outputs/4.0.0-impl/RELEASE-4.0.0.md`.
- `tests/ux/visibility.py` is the one red suite, unchanged by this release
  on purpose: it fails only on 3.2.0's map-share acceptance floors (a 5pp
  cost from the bottom sheet's own header replacing the floating 2.3.0
  entry pill), which is an owner call about those thresholds, not a defect.
- Android: **no new APK.** Nothing in 4.0.0 touches a native setting — the
  fourth page-zoom layer (`setSupportZoom(false)`) is untouched and the
  gate's `page zoom` check is green — so `android/app/build.gradle.kts`
  stays at `versionName "3.2.3"` / `versionCode 30203` per STANDARDS §14.8's
  precedent (3.2.3 was the one release that *did* need a cut, because it
  changed a native setting; this one changes none). If the owner wants the
  number bumped to match anyway, the one-file diff is recorded in
  `audit_outputs/4.0.0-impl/ANDROID.md` §6 — not applied here.
- Not verified this release: a real signed-in run (api.jpfoodmap.com /
  accounts.google.com unreachable from this sandbox), a real iOS Safari or
  Android device, a real installed PWA window, and the transit rail overlay's
  success path against live R2 (blocked by a bot challenge from this IP).
  Carried over unresolved from the integration/device passes; see
  `audit_outputs/4.0.0-impl/RELEASE-4.0.0.md` for the full list.
